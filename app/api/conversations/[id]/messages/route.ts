...
const parseRequestBody = async (request: Request): Promise<PostBody> => {
  try {
...
export async function POST(
  ...
  if (conversation.is_archived) {
    return NextResponse.json(
      { error: 'Cannot send messages in archived conversation' },
      { status: 400 },
    );
  }
  ...
  const userMessage = messageRepository.create({
    ...
    attachments: parsedBody.attachments ?? null,
  });
  ...
  try {
    ...
    // Conversation should still reflect the new user activity
